...
import org.ufa.mybatis.binding.PaginationMapperRegistry;
...
/**
 * <P>
 * 完全复制mybaits原来的SqlSessionFactoryBean，并修改使其能自动从多个配置文件中读取base
 * package信息，并组装成一个以英文逗号分隔的字符串，作为basePackage,作为TypeAliasesPackage，以支持org.apache.ibatis.type.Alias
 * </P>
 * <P>
 * 同时,由于mybatis的SqlSessionFactoryBean在buildSqlSessionFactory方法类,写死了Configuration的初始化,故覆盖之,
 * 注入能支持mapper分页的Configuration实现
 * </P>
 * 
 * @see org.mybatis.spring.SqlSessionFactoryBean
 *  
 */
public class UfaSqlSessionFactoryBean implements FactoryBean<SqlSessionFactory>, InitializingBean,
		ApplicationListener<ApplicationEvent>, ApplicationContextAware {
	private final Logger logger = LoggerFactory.getLogger(UfaSqlSessionFactoryBean.class);
	 ...
	 * any
	 * attempt at getting an SqlSession through Spring's MyBatis framework will throw an exception
	 * if
	...
	protected SqlSessionFactory buildSqlSessionFactory() throws IOException {
		...
		// 注入PaginationMapperRegistry,替换默认的MapperRegistry实例,使其支持mapper的分页
		ReflectionUtils.setFieldValue(configuration, "mapperRegistry", new PaginationMapperRegistry(configuration));
