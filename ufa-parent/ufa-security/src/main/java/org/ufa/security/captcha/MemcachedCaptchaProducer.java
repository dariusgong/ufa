 ...
 * @author Jonathan
 */
public class MemcachedCaptchaProducer extends AbstractCaptchaProducer {

	public static final String MEMCACHED_CAPTCHA_ID_SUFIX = "_captach";

	/**
	 * TODO
	 */
	private MemcachedCaptchaStoreClient captchaStoreClient;

	/**
	 * @return the captchaStoreClient
	 */
	public MemcachedCaptchaStoreClient getCaptchaStoreClient() {
		return captchaStoreClient;
	}

	/**
	 * @param captchaStoreClient the captchaStoreClient to set
	 */
	public void setCaptchaStoreClient(MemcachedCaptchaStoreClient captchaStoreClient) {
		this.captchaStoreClient = captchaStoreClient;
	}
	...
	protected void generateCaptcha(HttpServletRequest request, HttpServletResponse response) {
		...
		captchaStoreClient.storeCaptcha(id, captcha);
	...
	protected String generateTicketId(HttpSession session, HttpServletRequest request) {
		String id = super.generateTicketId(session, request);
		return id + MEMCACHED_CAPTCHA_ID_SUFIX;
	}
	...
	public boolean validateCaptcha(HttpServletRequest request, String captchaResponseText) {
		...
		boolean result = true;
		try {
			Captcha captcha = (Captcha) captchaStoreClient.getCaptcha(id);
			if (captcha == null)
				return false;
